...
from localstack.utils.ssl import install_predefined_cert_if_available, create_ssl_cert
...
def serve_gateway(
    ...
    """
    if isinstance(listen, HostAndPort):
        listen = [listen]
    ...
    for host_and_port in listen:
        ...
        if use_ssl:
            ...
            endpoint = endpoints.SSL4ServerEndpoint(
                reactor, host_and_port.port, context_factory
            )
        else:
